...
def riskmap_from_str(s):
    return np.array([list(map(int, list(line))) for line in s.splitlines()])
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Minimise risk")
    parser.add_argument(
        "input_file", metavar="INPUT_FILE", type=str, help="input file name"
    )
    parser.add_argument(
        "-s",
        "--scale-factor",
    ...
    parser.add_argument(
        "-p", "--print", action="store_true", help="print minimum risk path"
    )
